...
// The mat, the fifteen segments that stand in for skin and the reading off
// them all come from tools/mat-model.mjs, because two things read them: this
// solver and ruler-check, which scores the model against the real baked skin
// over the same blends blend-check judges. A copy here and a copy there is
// how this project last spent a round chasing a number the solver could not
// see. Why the model has this shape, and what it is worth, is written there.
...
// (SUNK comes from mat-model.mjs, alongside the segments it names.)
