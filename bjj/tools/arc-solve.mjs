...
//
// This used to be a table of "how far the skin hangs below this bone", nine
...
// runs from **13 to 46 cm** (the table said 10), because "shin" here is the
// knee and its skin reaches the ankle: how far down it goes depends on how the
// leg is bent. No constant is within a hand's breadth of right.
//
...
// baked skin over 188 samples per bone, and scored against it: 1.0 to 3.7 cm of
// mean error where the old table was out by twelve, for the same nine
// multiplications. The chest is the worst of them at 11 cm in the tail, and
// that is the model's ceiling — it is a straight limb approximating a torso.
const SKIN_SEG = {
...
// Every bone whose skin can be the thing that goes through the mat. The old
// list had no knee in it, which is the one part most likely to be under the
// floor of a transition: the foot clamp holds the ankle at a fixed height and
// the two-bone solve puts the knee where it likes.
const SUNK = Object.keys(SKIN_SEG);
...
function measure(from, to) {
  ...
  for (let i = 0; i < STEPS; i++) {
    ...
    for (const role of ['A', 'B']) {
      for (const b of SUNK) {
        const under = skinUnder(rig.skel[role], b);
        if (under > 0) {
          sum += under * under * 20;
